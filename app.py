...
def player(id):
    connect = sqlite3.connect('Bball.db')
    c = connect.cursor()
    player = c.execute("""SELECT Player.id, Player.Firstname,Player.Lastname, Player.Imagefilename, Team.teamname FROM Player JOIN Team ON Player.Teamid=Team.team_id
